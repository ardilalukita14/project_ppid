            ...
            <div class="card" style="padding-left:20px; padding-right:20px; padding-top:20px;">
                ...
                <br>
                      <div class="card-tools">
                            <br>
                            <a href="{{ route('informasi_publik.daftar_informasi.publik.create') }}" class="btn btn-primary btn-round">Tambah Data <i class="fa fa-plus"></i></a>
                      </div>
                                    ...
                                    <table class="display table table-striped table-hover" id="dataTable">
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($informasi_publik as $pb)
                                                ...
                                                <td>{!!$pb->link!!}</td>
                                                <td>
                                                    <form action="{{ route('informasi_publik.daftar_informasi.publik.destroy',$pb->id) }}"  method="POST">
                                                        <a href="{{ route('informasi_publik.daftar_informasi.publik.edit',$pb->id) }}" class="btn btn-warning "><i class="fa fa-edit"></i> Ubah</a>
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-danger" onclick="return confirm('Konfirmasi hapus Daftar Informasi Publik ?')" ><i class="fas fa-trash"></i> Hapus</button>
                                                    </form>
                                                </td>
                                            </tr>
                                            @endforeach
