                ...
                <div class="card-body">
                    ...
                    <form class="form-valide" action="{{route('kategori.edit',$kategori->id)}}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-8">
                                    <input type="text" class="form-control" id="nama_kategori" name="nama_kategori" required="" value="{{$kategori->nama_kategori}}">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label class="col-lg-3 col-form-label" for="status">Status</label>
                                <div class="col-sm-8">
                                <select class="form-control" name="status" value="{{$kategori->status}}">
                                    @if ($kategori->status == 1)
                                    <option value="{{$kategori->status}}" selected>Aktif</option>
                                    @elseif ($kategori->status == 0)
                                    <option value="{{$kategori->status}}" selected>Tidak Aktif</option>
                                    @endif
